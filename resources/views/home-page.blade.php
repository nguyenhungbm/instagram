    ...
    <section class="sd">
        ...
        <div class="csa">
                    ...
                    <div class="list-follow">
                        ...
                        @else
                            <a href="{{ route('chat.show', $user->id) }}"
                               class="message">{{ __('translate.Message')}}</a>
                            <a class="unfollow follows{{$user->id}}" href="javascript:"
            ...
            <div class="csc">
                ...
                <p class="cs" id="myBtn-6"><b class="follower">{{count($userFollow)}}</b> {{ __('translate.followers')}}
                </p>
                <!-- modal follow -->
                <div id="myModal-6" class="modal">
                    <div class="modal-content settings animate__animated animate__zoomIn">
                        <li class="one">{{ ucwords(__('translate.followers'))}} <span class="float-right cs" id="exit6"
                                                                                      style="right: 15px;position: absolute;top: 0;">&times;</span>
                        </li>
                        <div class="settingss">
                            @if(!count($userFollow))
                                ...
                                @foreach($userFollow as $list)
                                    <li class="clr user{{$list->user_id}}" style="height: 50px;">
                                        ...
                                        @if($list->user_id!=Auth::id())
                                            @if(Follow::checkFollow(Auth::id(), $list->user_id))
                                                @if($user->id != Auth::id())
                                ...
                                @foreach($areFollow as $key=> $list)
                                    <li class="clr users{{$list->friends->id}}" style="height: 50px;">
                                        ...
                                        @if($list->friends->id!=Auth::id())
                                            @if(Follow::checkFollow(Auth::id(), $list->friends->id))
                                                @if($user->id != Auth::id())
    ...
    <div class="image d-none">
        ...
        <div class="second d-none">
            <form action="{{ route('post.profile')}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="title ">
                    <a href="javascript:" class="back"><i class="fa fa-long-arrow-left"></i> {{ __('translate.Back')}}
                    </a>
                    <p>{{ __('translate.New Post')}}</p>
