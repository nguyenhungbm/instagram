...
@section('content')
    ...
    <div class="asd">
        ...
        <div class="ass">
            <div class="heq">
                <div class="hew"><a href="{{ route('get.home-page', $val->user->user)}}"><img
                                src="{{ pare_url_file($val->user->avatar, 'user') }}" class="avatar_user_uploaded"></a>
                </div>
                <div class="hee">
            ...
            <div class="her hdl{{$val->id}}" id="hell">
                ...
                @if($val->p_content!='')
                    <div class="clr het">
                        ...
                        <div class="hew"><a href="{{ route('get.home-page', $val->user->user)}}"><img
                                        src="{{ pare_url_file($val->user->avatar, 'user') }}"
                                        class="avatar_user_uploaded"></a></div>

                        <div class="hep">
                            <p><a href="{{ route('get.home-page', $val->user->user)}}"><b>{{$val->user->c_name}}
                                </a> </b> {{$val->p_content}}</p>
                        </div>
                ...
                <div class="list-comment{{$val->id}}">
                    @foreach(Comment::where('c_post', $val->id)->orderBy('created_at', 'desc')->get() as $value => $cmt)
                        <div class="clr het hjk{{$value}} " style="display:none">
                            <div class="hew"><a href="{{ route('get.home-page', $cmt->users->user)}}"><img
                                            src="{{ pare_url_file($cmt->users->avatar, 'user') }}"
                                            class="{{ $cmt->c_user_id ==Auth::id() ? 'avatar_user_uploaded' :''}}"></a>
                            </div>
                            <div class="hep">
                                <p>
                                    <b><a href="{{ route('get.home-page', $cmt->users->user)}}">{{$cmt->users->c_name}}</a></b>{{ $cmt->c_comment}}
                                </p>
                            </div>
                            <i class="fa fa-ellipsis-h"></i>
                ...
                <div class="buttons">
                    <button class="button{{$val->id}} ">+</button>
                </div>
                <script>
                    ...
                    currentindex = 0;
                    maxindex = "{{Comment::where('c_post', $val->id)->count()}}";
    ...
    @if(count($related_post))
        <br>
        <div class="posts">
            <p class="text-center">Thêm các bài viết từ <a
                        href="{{route('get.home-page', $val->user->user)}}"><b>{{$val->user->c_name}}</b></a></p>
            <br>
            <div class="clr">
                @foreach($related_post as $key=> $val)
                    <div class="cs cse {{ $key%3 == 1 ? 'uio' : '' }}" id="myBtnn{{$val->id}}">
                        ...
                        <img src="{{ pare_url_file($val->p_image, 'profile/img_small') }}" class="img_small"
                             id="image{{$key}}">
                    </div>
                @endforeach
            </div>
