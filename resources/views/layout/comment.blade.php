<form action="{{ route('comment.post')}}" class="clr position-relative ">
    @csrf
    <textarea autocomplete="off" id="coms" class="textarea-{{$value}} textarea-comment{{$value}} textarea-comment"
              placeholder="{{ __('translate.Add a comment')}}..."></textarea>
    <input type="hidden" value="{{$value}}" class="post-comment{{$value}}">
    <button type="button" id="submit-{{$value}}" class="  button-comment disabled position-relative">
