                ...
                <div class="list-comment{{$val->id}}">
                    @foreach(Comment::where('c_post', $val->id)->orderBy('created_at', 'desc')->get() as $value => $cmt)
                        <div class="clr het hjk{{$value}} " style="display:none">
                            ...
                            <div class="hep">
                                <p>
                                    <b><a href="{{ route('get.home-page', $cmt->users->user)}}">{{$cmt->users->c_name}}</a>
                                    </b> {{$cmt->c_comment}}</p>
                            </div>
                            <i class="fa fa-ellipsis-h"></i>
                            <div class="os heo">{{ $cmt->created_at->diffForHumans() }} </div>
                            <input type="hidden" id="com{{ $val->id }}"
                                   value="{{Comment::where('c_post', $val->id)->count()}}">
                        </div>
                    @endforeach
                </div>
                @if(Comment::where('c_post', $val->id)->count() > 5)
                    <div class="buttons">
