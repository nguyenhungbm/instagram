    ...
    <div class="messages  d-block">
        <div class="left d-inline-block" style="height: 100%;width:35%; ">
            ...
            <div class="bottom-left">
                <ul>
                    @if(isset($friend->id))
                        @if(!count(Chat::where(['user_id'=>Auth::id(), 'friend_id'=>$friend->id])->get())
                        &&
                        !count(Chat::where(['friend_id'=>Auth::id(), 'user_id'=>$friend->id])->get())
                        )
                            <a href="{{ route('chat.show', $friend->id) }}">
                                <li class="clr">
                                    ...
                                    <onlineuser v-bind:friend="{{$friend }}"
                                                v-bind:onlineusers="onlineUsers"></onlineuser>
                                </li>
                            </a>
                        @endif
                    @endif
                        ...
                        @foreach($chat as $list)
                            ...
                            <a href="{{ route('chat.show', $userr->id) }}">
                                <li class="clr">
                                    ...
                                    <onlineuser v-bind:friend="{{ $userr }}"
                                                v-bind:onlineusers="onlineUsers"></onlineuser>
                                </li>
                            </a>
                        @endforeach
    ...
    <div id="id01" class="w3-modal d-none">
        <div class="w3-modal-content animate__flipInX d-flex po">
            ...
            <div class="pi">
                ...
                <input type="text" name="key" placeholder="{{ __('translate.Search')}}..." id="search"
                       autocomplete="off">
            </div>
            ...
            <div class="pu ">
                ...
                @foreach($chat as $key=>$list)
                    ...
                    <div class="clr py cs py{{$list->id}}">
                        ...
                        <button class="cs hihi{{$list->id}}"><i class="fa fa-lg fa-check haha{{$list->id}}"></i>
                        </button>
                    </div>
                    <script>
                        $(function () {
